...
module ParseMarkdown.FormatText.TryAddBasicList (
    tryAddBasicToList, 
    fillInvalidPattern, 
    parseBasicChar
) where
...
fillInvalidPattern index limit c dataText
    ...
    | otherwise =
        fillInvalidPattern (index + 1) limit c (endData { precedentChar = c })
    where
...
tryAddBasicToList dataText
    | length (basicStr dataText) > 0 =
        dataText { listText = (listText dataText) ++
        [(TString (basicStr dataText))], basicStr = "" }
    | otherwise = dataText
