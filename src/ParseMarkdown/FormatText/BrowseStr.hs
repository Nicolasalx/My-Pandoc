...
browseStr (x:xs) dataText hasTakeFrstChar
    | not hasTakeFrstChar = 
        ...
        browseStr (fst (detectSymbol ([x] ++ xs) (dataText {
          precedentChar = x }))) (snd (detectSymbol ([x] ++ xs)
                    (dataText { precedentChar = x }))) True
    | otherwise = snd (detectSymbol xs (dataText { precedentChar = x })) `seq` 
