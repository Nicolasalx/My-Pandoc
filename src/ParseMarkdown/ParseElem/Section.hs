...
createSection :: Int -> String -> DataParsing ->
    [PContent] -> ([PContent], DataParsing)
createSection actualLevel titleSection dataParsing allContent
    | levelSection dataParsing == 0 = createNewSection actualLevel
        (skipSpaces 100 titleSection) dataParsing allContent True
    | (levelSection dataParsing) >= actualLevel = createNewSection actualLevel
        (skipSpaces 100 titleSection) dataParsing allContent True
    | otherwise = createNewSection (actualLevel - (levelSection dataParsing))
        (skipSpaces 100 titleSection) dataParsing allContent False
