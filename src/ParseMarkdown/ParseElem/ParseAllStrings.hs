...
parseAllString :: [String] -> DataParsing -> [PContent] -> (Either String [PContent], DataParsing)
parseAllString [] dataParsing allContent = ((Right allContent), dataParsing)
parseAllString (x:xs) dataParsing allContent = do
    let (content, stringParsed) = parseEachString x x dataParsing False allContent
        (newContent, newData) = tryAddElemToContent stringParsed content
        newDataParsed = newData { nbReturnLines = nbReturnLines dataParsing + 1 }
